...
export class DanmakuSendBoxControlItem implements Disposable {
  ...
  constructor(container: HTMLElement, private player: Player) {
    ...
    utils = _utils;
    const { $, strToDom, addDisposableListener } = utils;

    this.element = container.appendChild($('.danmaku_send'));
    const settingElement = this.element.appendChild($());
    settingElement.appendChild(strToDom(`<svg class="rplayer_icon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    this.typeCBs.scroll = new _components.Checkbox(rowElement, { html: '滚动', checked: true, change: this.onTypeChange('scroll') });
    this.typeCBs.top = new _components.Checkbox(rowElement, { html: '顶部', change: this.onTypeChange('top') });
    this.typeCBs.bottom = new _components.Checkbox(rowElement, { html: '底部', change: this.onTypeChange('bottom') });
    rowElement = row();
    ...
    this.colorInputElement.addEventListener('input', () => {
    ...
    rowElement.addEventListener('click', ({ target }: MouseEvent) => {
  ...
  send = () => {
    ...
    const bullet: BulletOption = {
      ...
      color: isDefaultColor(value) ? undefined : value,
    };
    this.player.emit('danmaku-send', bullet);
